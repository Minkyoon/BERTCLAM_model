...
class CLAM_SB(nn.Module):
    ...
    def forward(self, h, tabular, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        #_, topk_indices = torch.topk(A_fi, k=512, dim=0)  # [k]
               
        #critical_instances = h[topk_indices, :]  # Use advanced indexing to extract the critical instances
        # Reduce dimension
        reduced_instances = self.dim_reduction(h)  # [k, 768]
        ...
        logits = self.classifier_tr(aggregated_output)  # [1, num_classes]     
        ...
        return logits, Y_prob, Y_hat, A_raw, results_dict, logits.squeeze(0) 
