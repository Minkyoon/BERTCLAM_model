...
#         self.total_attention = nn.ModuleList([nn.Linear(self.total_feature_dim , self.total_feature_dim ) for _ in range(num_heads)])

#     def forward(self, image_feature, table_feature):
#         attended_features = []
#         score =[]
#         uni_feature = torch.cat([image_feature, table_feature], dim=1)
#         for i in range(self.num_heads):
#             # image_feature에 대한 attention score 계산
            
            
        
#             total_attention_score = torch.softmax(self.total_attention[i](uni_feature), dim=1)

#             attended_uni_feature = total_attention_score * uni_feature
            
#             score.append(total_attention_score)
            

#             # attended_image_feature와 attended_table_feature를 합침
            
#             attended_features.append(attended_uni_feature)
...
class CLAM_SB(nn.Module):
    def __init__(self, gate = True, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        ...
        if gate:
            attention_net = Attn_Net_Gated(L = 1024, D = 512, dropout = dropout, n_classes = 1)
        else:
            attention_net = Attn_Net(L = size[1], D = size[2], dropout = dropout, n_classes = 1)
        fc.append(attention_net)
        self.attention_net = nn.Sequential(*fc)
        self.attention_module=MultiHeadAttention(1024, 49,1 )
        self.classifiers = nn.Linear(size[1], n_classes)
        # self.classifiers = nn.Linear(size[1] + tabular_size, n_classes)
        instance_classifiers = [nn.Linear(1024, 2) for i in range(n_classes)]
        self.instance_classifiers = nn.ModuleList(instance_classifiers)
        self.k_sample = k_sample
        self.critical_instance_classifier = nn.Sequential(
            nn.Linear(1024, 512),  # instance_feature_size is 2048, hidden_layer_size is 512 (example)
            nn.ReLU(),
            nn.Linear(512, 2)  # num_output_classes is 2
        )
    ...
    def set_classifier(self, tabular_size : int) : 
        self.classifiers = nn.Linear((self.size_dict[self.size_arg][0] + tabular_size)*1, self.n_classes)
    ...
    def forward(self, h, tabular, label=None, instance_eval=False, return_features=False, attention_only=False):
        ...
        critical_instance_indices = torch.argmax(A_fi, dim=0)  # Find the index of the maximum attention score for each sample in the batch
        critical_instances = h[critical_instance_indices, :]  # Use advanced indexing to extract the critical instances
        
        critical_instance_logits = self.critical_instance_classifier(critical_instances)
        ...
        return logits, Y_prob, Y_hat, A_raw, results_dict, critical_instance_logits
